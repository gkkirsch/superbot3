...
import { useSpace, useSpaceMessages } from '@/hooks/useSpaces'
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b">
        <h1 className="text-base font-semibold text-foreground">{space.slug}</h1>
        <Badge variant={space.running ? 'success' : 'secondary'}>
          {space.running ? 'Running' : 'Stopped'}
        </Badge>
        {space.codeDir && (
          <span className="text-xs text-stone ml-auto">{space.codeDir}</span>
        )}
      </div>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col min-w-0 border-r">
          <ChatSection
            messages={messages || []}
            sendFn={(text) => sendSpaceMessage(slug!, text)}
            queryKey={['space-messages', slug!]}
          />
        </div>

        {/* Tabs — secondary */}
        <div className="w-full lg:w-96 xl:w-[28rem] overflow-y-auto scrollbar-thin">
